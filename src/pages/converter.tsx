...
const convertCommands = (text: string): string => {
  ...
  for (const cmd of commandsMap) {
    if (['＊運動', 'スピード', 'ストレングス', 'アクロバット', 'ダイブ', '＊格闘', '＊投擲'].includes(cmd.skill) || cmd.skill.startsWith('武術') || cmd.skill.startsWith('★奥義') || cmd.skill.startsWith('★射撃')) {
...
const convertCcfolia = (text: string): string => {
  ...
  for (const param of jsonObj.data.params) {
    srcParams[param.label] = parseInt(param.value);
  }
    ...
    <Layout
      ...
      <main>
        <div className={clsx('container', style.converter)}>
          <p>エモクロアTRPGのキャラクターシートから、チャットパレットまたはCCFOLIA形式でコピーしたものを上に貼り付けて「コンバート」を押してください。</p>
          <textarea id="input" className={style.textarea} value={inputText} onChange={onChangeInput} />
          <button id="convert" className="button button--primary button--lg" onClick={() => handleConvert()}>コンバート</button>
          <textarea id="output" className={style.textarea} readOnly value={outputText} />
          <button id="copy" className="button button--secondary button--lg" onClick={() => handleCopy()}>{copyButtonText}</button>
        </div>
      </main>
    </Layout>
  );
